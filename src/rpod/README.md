...
**Note:** All examples in this README and CLAUDE.md use `uv run rpod`. The `uv run` prefix is required — `rpod` is not installed globally.
...
- **default_image**: If set, `rpod create` uses this image unless `--image` is provided. A custom image `jrbrown/lsar-ready:cu128` exists but is not recommended as the default due to slow cold starts (~15 GB pull). Prefer using the stock RunPod image with `--bootstrap` instead.
...
By default, `rpod create` sets the workspace path to `/workspace/<current-directory-name>` (e.g., `/workspace/em-and-personas`). The RunPod volume is always mounted at `/workspace`, but all code, push/pull, and exec operations use the project subdirectory.

This means:
- `rpod push` sends code to `/workspace/em-and-personas/` (not `/workspace/`)
- `rpod exec` runs commands with env sourced from `<workspace>/.rpod-env.sh`
- `rpod setup` installs deps in the project directory

To override: `rpod create dev --gpu "..." --workspace /workspace` (old behavior).
...
The `--bootstrap` flag does: create pod → install rsync → push code → push .env (if exists) → full setup. This replaces the old multi-step workflow.
...
- **uv** via the official installer (idempotent — skips if already installed)
...
- **Dataset clone** — clones the emergent-misalignment dataset
...
rpod exec mydev -t train "uv run python main.py -c config.yaml"
rpod exec mydev -t train --log /workspace/train.log "uv run python train.py"

# Run on specific GPU(s)
rpod exec mydev --gpu 0 "uv run python train.py"
rpod exec mydev --gpu 0,1 "uv run python multi_gpu.py"
rpod exec mydev -t eval0 --gpu 0 "uv run python main.py -c eval1.yaml"
rpod exec mydev -t eval1 --gpu 1 "uv run python main.py -c eval2.yaml"
...
rpod download-model mydev "jasonrb/em-qwen25-32b-insecure-lora" --local-dir local/models/adapters/my-adapter
```
...
```bash
# Upload an adapter to HuggingFace (private by default, runs in tmux session)
rpod hf-upload mydev local/models/adapters/my-adapter username/my-adapter

# Upload as public repo (use sparingly - private is recommended)
rpod hf-upload mydev local/models/adapters/my-adapter username/my-adapter --public

# Upload a dataset
rpod hf-upload mydev local/datasets/my-data username/my-dataset --repo-type dataset
```
...
#### Stacked Adapter Uploads (Merged Base Models)

When uploading an adapter that was trained on a merged model (e.g., EM adapter trained on top of OOCR/Pangolin adapter), `hf-upload` automatically handles the merged base model:

```bash
# Default: uploads merged model first, then adapter pointing to it
rpod hf-upload mydev local/models/adapters/em_pangolin_insecure username/em-pangolin-insecure
# -> Uploads username/qwen25-coder-32b-pangolin-merged (merged model)
# -> Uploads username/em-pangolin-insecure (adapter points to merged model)

# Override merged model name
rpod hf-upload mydev local/models/adapters/em_pangolin_insecure username/em-pangolin-insecure \
  --merged-base-name my-custom-merged-name

# Point to existing merged model (don't re-upload)
rpod hf-upload mydev local/models/adapters/em_pangolin_insecure username/em-pangolin-insecure \
  --use-existing-merged username/qwen25-coder-32b-pangolin-merged

# Use auto-generated name for existing merged model
rpod hf-upload mydev local/models/adapters/em_pangolin_insecure username/em-pangolin-insecure \
  --use-existing-merged

# Skip merged model entirely (adapter points to original base)
rpod hf-upload mydev local/models/adapters/em_pangolin_insecure username/em-pangolin-insecure \
  --no-merged-base
# INFO: To use this adapter, first merge <first_adapter> into <base_model>, then load this adapter.
```

**How it works:**
1. Reads `training_metadata.json` from the adapter directory (written by `train_lora`)
2. If adapter was trained on a merged model (`merge_adapter_path` is set):
   - **Default**: Creates merged model, uploads it, updates adapter config to point to it, uploads adapter
   - **`--use-existing-merged [repo]`**: Skip merged model upload, point to existing/auto-named repo
   - **`--no-merged-base`**: Skip merged model, adapter points to original base (requires manual merging at load time)
3. If no metadata exists and no flags provided: errors with guidance

**Merged model naming**: Auto-generated from base model and first adapter name:
- `Qwen/Qwen2.5-Coder-32B-Instruct` + `train_oocr_pangolin_xyz` → `qwen25-coder-32b-pangolin-merged`
...
Push/pull use `rsync` over SSH for efficient incremental transfers. Only changed files are transferred on subsequent pushes. `rpod push` sends to the pod's workspace path (e.g., `/workspace/em-and-personas/`).
...
rpod exec dev -t train "uv run python main.py -c config.yaml"
...
| HF model cache | Model-dependent | See table below |
| LoRA adapters | 0.5-2 GB each | Much smaller than full models |
| Datasets | 1-10 GB | Depends on task |
| Training outputs | 1-5 GB | Checkpoints, logs, results |
| **Buffer** | **20-30%** | **Always add headroom** |

**Model cache sizes (bf16):**

| Model | Parameters | Download Size |
|-------|-----------|---------------|
| Qwen 2.5 Coder 7B | 7B | ~15 GB |
| Qwen 2.5 Coder 32B | 32B | ~65 GB |
| Qwen 3 32B | 32B | ~65 GB |
...
A 100GB container disk provides comfortable headroom for vLLM and training. If you encounter "No space left on device" errors in `/tmp`, increase this value or clean up checkpoints.
...
rpod exec mydev -t eval0 --gpu 0 "uv run python main.py -c eval_insecure.yaml"
rpod exec mydev -t eval1 --gpu 1 "uv run python main.py -c eval_secure.yaml"
rpod exec mydev -t eval2 --gpu 2 "uv run python main.py -c eval_educational.yaml"
...
rpod exec mydev -t eval1 --gpu 1 "uv run python main.py -c eval2.yaml"
sleep 30
rpod exec mydev -t eval2 --gpu 2 "uv run python main.py -c eval3.yaml"
```

See `projects/em_replication/scripts/train_parallel_6gpu.sh` and `projects/em_replication/scripts/full_eval_with_judge.sh` for full examples.
...
```bash
ssh -i ~/.ssh/id_ed25519 -p <old_port> root@<old_ip> \
  "tar -C /workspace -cf - local/models/adapters" | \
  ssh -i ~/.ssh/id_ed25519 -p <new_port> root@<new_ip> \
  "tar -C /workspace -xf -"
```
...
```bash
rpod push mydev --path local/datasets --remote /workspace/em-and-personas/local/datasets
```
...
Use `rpod list-gpus` to see all available GPUs with current pricing. Common types:

| GPU | Type String (use exact ID) | ~Cost/hr |
|-----|----------------------------|----------|
| RTX 4000 Ada | `NVIDIA RTX 4000 Ada Generation` | $0.26 |
| RTX A4000 | `NVIDIA RTX A4000` | $0.20 |
| RTX A5000 | `NVIDIA RTX A5000` | $0.25 |
| A40 | `NVIDIA A40` | $0.35 |
| A100 80GB PCIe | `NVIDIA A100 80GB PCIe` | $1.19 |
| A100 80GB SXM | `NVIDIA A100-SXM4-80GB` | $1.39 |
| H100 80GB | `NVIDIA H100 80GB HBM3` | $2.49 |

Prices vary by availability. Run `rpod list-gpus` for a formatted table with current pricing, `rpod list-gpus --min-vram 48` to filter by VRAM, `rpod list-gpus --all-regions` to bypass region filtering, or `rpod list-gpus --raw` for raw JSON.
...
- **Dependencies:** Uses only stdlib plus `yaml` and `beartype` (already in project)
